          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] sm:text-[10px] text-gray-600 whitespace-nowrap flex items-center gap-1">
                <JapanFlag className="w-3 h-3 inline-block" /> Direct Import
                from Japan to Sri Lanka{" "}
                <SriLankaFlag className="w-3 h-3 inline-block" />
              </span>
            </div>
          </div>
...
const AppContent: React.FC = () => {
  ...
  const handleAddCar = async (newCar: Omit<Car, "id">) => {
    try {
      ...
      alert(
        "Failed to add car. Please check your internet connection and try again."
      );
    }
  };
  ...
  const handleEditCar = async (id: number, updatedCar: Partial<Car>) => {
    try {
      ...
      setCars(
        cars.map((car) => (car.id === id ? { ...car, ...updatedCar } : car))
      );
      alert("Car updated successfully!");
    } catch (error) {
      console.error("Failed to edit car:", error);
      alert(
        "Failed to update car. Please check your internet connection and try again."
      );
    }
  };
  ...
  const handleDeleteCar = async (id: number) => {
    ...
    try {
      ...
      alert(
        "Failed to delete car. Please check your internet connection and try again."
      );
    }
  };
